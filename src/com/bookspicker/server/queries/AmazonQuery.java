...
public class AmazonQuery implements BookstoreQuery {
  ...
  /**
   * Returns general book information based on the ISBN (image url + author + title and etc).
   * Should be used to query book info when we don't have it in the cache, and if we already have the 
   * ISBN. 
   */
  ...
  private void extractBookOffers(Bundle bundle, Book book, String request) {
    ...
    if (mainDocument == null) {
      ...
      if (offerDocument == null) {
        ...
        StoreName shopName = sellerName.equals("Amazon.com") ? 
            StoreName.AMAZON : StoreName.AMAZON_MARKETPLACE;
        bundle.addOffer(book, new OnlineOffer(price, SHIPPING_COST, shopName, sellerName, condition, url));
      }
    }
  }
...
//    logger.debug("done adding books");
  ...
  private Book extractBookInfo(Element book) {
    //Element bookAttributes = (Element) book.getElementsByTagName("ItemAttributes");
